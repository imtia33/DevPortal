// Import multiple icon families from Expo Vector Icons
const ioniconsMap = {
  "accessibility": 59905,
...
const materialCommunityIconsMap = {
  "ab-testing": 983497,
  "abacus": 988896,
  "abjad-arabic": 987944,
  "abjad-hebrew": 987945,
...
export const getIconName = (family, iconName) => {
  const iconFamily = ICON_FAMILIES[family];
  if (!iconFamily) return iconName;
  
  // Check if icon exists with prefix
  if (iconFamily.map[iconFamily.prefix + iconName]) {
    return iconFamily.prefix + iconName;
  }
  
  // Check if icon exists with fallback prefix
  if (iconFamily.fallbackPrefix && iconFamily.map[iconFamily.fallbackPrefix + iconName]) {
    return iconFamily.fallbackPrefix + iconName;
  }
  
  // Check if icon exists without prefix
  if (iconFamily.map[iconName]) {
    return iconName;
  }
  
  return iconName; // Return original if not found
};

export const getAllIcons = () => {
  const allIcons = {};
  
  Object.entries(ICON_FAMILIES).forEach(([familyKey, family]) => {
    Object.keys(family.map).forEach(iconName => {
      allIcons[iconName] = {
        name: iconName,
        family: familyKey,
...
// Get icons by family
export const getIconsByFamily = (family) => {
  const iconFamily = ICON_FAMILIES[family];
  if (!iconFamily) return {};
  
  return Object.keys(iconFamily.map).map(iconName => ({
    name: iconName,
    family: family,
    familyName: iconFamily.name
  }));
};
